...
const ViewActivity = () => {
  ...
  const handleFilter = async () => {
    ...
    try {
      ...
      if (selectedEmail) filters.email = selectedEmail;
  ...
  const handleView = (id) => {
    ...
    navigate("/ApprovedView", { state: { userId: id } });
  };
    ...
    <div className="container mt-4 mb-5 p-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end gap-4">
        ...
        <div className="flex-1">
          <label className="block mb-1 text-gray-700 font-medium">Status <span className="text-red-500">*</span></label>
          <select
            value={status}
            onChange={(e) => setStatus(e.target.value)}
            className={`w-full border rounded-md px-3 py-2 ${!status ? "border-red-500" : "border-gray-300"
              }`}
          >
